...
function montaSelectOMAmazonia(idForca, qtdeSvAmz){
    FacadeAjax.getOmsAmazonia(idForca, {
        callback: function(oms){
            dwr.util.removeAllOptions("txtOMNomeSvAmz" + qtdeSvAmz);
            dwr.util.addOptions("txtOMNomeSvAmz" + qtdeSvAmz, [{id: "0", nome: "Selecione uma OM..."}], "id", "nome");
            dwr.util.addOptions("txtOMNomeSvAmz" + qtdeSvAmz, oms, "id", "nome");
        } 
    });
}
...
$("#btnAddSvAmz").click(function(){      
    $("#fieldsetSvAmz").append("<div id=linha"+qtdeSvAmz+" class=\"linha fieldset-interno\">"+
                                ...
                                "            <select class=\"form-control\" id=txtOMNomeSvAmz"+qtdeSvAmz+" name=\"txtOMNomeSvAmz\">"+
                                "               <option value=\"0\" selected>Selecione uma OM...</option>"+                    
                                "            </select>"+                    
                                "            <div class=\"valid-feedback\">Selva!</div>"+
                                "            <div class=\"invalid-feedback\">Campo Obrigatório!</div>"+
                                "        </div>"+ 
                                ...
                                "    </div>"+  
                                "</div>");
    montaSelectOMAmazonia($("select[name=txtForca]").val(), qtdeSvAmz);
